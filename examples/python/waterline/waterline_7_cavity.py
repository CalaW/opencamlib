    ...
    for lop in loops:
        ...
        for p in lop:
            if n == 0:  # don't draw anything on the first iteration
                ...
                myscreen.addActor(
                    camvtk.Line(
                        p1=(previous.x, previous.y, previous.z), p2=(p.x, p.y, p.z), color=loopcolor
                    )
                )  # the normal line
        ...
        if len(lop) == 2:
            for p in lop:
                print(p)
                myscreen.addActor(
                    camvtk.Sphere(center=(p.x, p.y, p.z), radius=0.1, color=camvtk.green)
                )
        nloop = nloop + 1
...
if __name__ == "__main__":
    ...
    stl = camvtk.STLSurf(Path(__file__).parent / "../../../stl/sphere_cutout.stl")
