...
def drawFiber(myscreen, f, fibercolor=camvtk.red):
    ...
    for i in inter:
        if not i.empty():
            ...
            ballradius = 0.0005
            myscreen.addActor(
                camvtk.Line(p1=(ip1.x, ip1.y, ip1.z), p2=(ip2.x, ip2.y, ip2.z), color=fibercolor)
            )
            myscreen.addActor(
                camvtk.Sphere(
                    center=(ip1.x, ip1.y, ip1.z),
                    radius=ballradius,
            ...
            cc2 = i.upper_cc
            myscreen.addActor(
                camvtk.Sphere(center=(cc1.x, cc1.y, cc1.z), radius=ballradius, color=camvtk.lgreen)
            )
            myscreen.addActor(
                camvtk.Sphere(center=(cc2.x, cc2.y, cc2.z), radius=ballradius, color=camvtk.lgreen)
            )
            # cutter circle
    ...
    for lop in loops:
        ...
        for p in lop:
            if n == 0:  # don't draw anything on the first iteration
                ...
                myscreen.addActor(
                    camvtk.Line(
                        p1=(previous.x, previous.y, previous.z + zofz),
                        p2=(p.x, p.y, p.z + zofz),
                        color=loopColor,
                    )
                )  # the normal line
                ...
                myscreen.addActor(
                    camvtk.Line(
                        p1=(p.x, p.y, p.z + zofz),
                        p2=(first_point.x, first_point.y, first_point.z + zofz),
                        color=loopColor,
                    )
                )
            else:
        ...
        if len(lop) == 2:
            for p in lop:
                print(p)
                myscreen.addActor(
                    camvtk.Sphere(center=(p.x, p.y, p.z + zofz), radius=0.0005, color=camvtk.pink)
                )
        nloop = nloop + 1
