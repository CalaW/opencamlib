...
std::pair<Vertex, Vertex> SimpleWeave::find_neighbor_vertices(VertexPair v_pair, Interval& ival) {
    ...
    Interval::VertexPairIterator v_above =
        itr; // lower_bound returns one beyond the give key, i.e. what we want
    ...
    for (Fiber& xf : xfibers) {
        ...
        for (Interval& xi : xf.ints) {
            ...
            if ((xmax - xmin) > 0) {
                    ...
                    if ((xmin < yf.p1.x) && (yf.p1.x < xmax)) {
                        ...
                        for (Interval& yi : yf.ints) {
                            ...
                            if ((ymin < xf.p1.y) && (xf.p1.y < ymax)) {
                                ...
                                // X interval xi on fiber xf intersects with Y interval yi on fiber
                                // yf intersection is at ( yf.p1.x, xf.p1.y , xf.p1.z )
                                ...
                                boost::tie(x_u, x_l) =
                                    find_neighbor_vertices(VertexPair(v, v_position.x), xi);
                                // std::cout << "found: x_u , x_l : " << x_u << " , " << x_l <<
                                // "\n";
                                Vertex y_u, y_l;
                                boost::tie(y_u, y_l) =
                                    find_neighbor_vertices(VertexPair(v, v_position.y), yi);
                ...
                // if there isn't a single intersecting interval, then remove the x-interval as it
                // is useless
