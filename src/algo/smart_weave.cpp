...
// given a VertexPair and an Interval, in the Interval find the Vertex above and below the given
// vertex
std::pair<Vertex, Vertex> SmartWeave::find_neighbor_vertices(VertexPair v_pair, Interval& ival,
                                                             bool above_equality) {
    Interval::VertexPairIterator itr = ival.intersections2.lower_bound(
        v_pair); // returns first that is not less than argument (equal or greater)
                ...
                for (; current != xi->intersections_fibers.end(); current++) {
                    ...
                    if ((*current - *prev) > 1) {
                        std::vector<Interval>::iterator yi =
                            find_interval_crossing_x(xf, *(*prev + 1));
        ...
        for (yi = yf.ints.begin(); yi < yf.ints.end(); yi++) {
            ...
            if (yi->intersections_fibers.size() > 1) {
                ...
                for (; current != yi->intersections_fibers.end(); current++) {
                    if ((*current - *prev) > 1) {
                        std::vector<Interval>::iterator xi =
                            find_interval_crossing_y(*(*prev + 1), yf);
...
Vertex SmartWeave::add_cl_vertex(const Point& position, Interval& ival, double ipos) {
    ...
    g[v].type = CL;
    ival.intersections2.insert(
        VertexPair(v, ipos)); // ?? this makes Interval depend on the WeaveGraph type
    ...
    for (yf = yfibers.begin(); yf < yfibers.end(); yf++) {
        ...
        for (yi = yf->ints.begin(); yi < yf->ints.end(); yi++) {
            ...
            while ((xf < xfibers.end()) && !is_crossing) {
                xf++;
                if (xf < xfibers.end())
                    is_crossing = crossing_y(*xf, xi, *yi, *yf);
            }
            ...
            if (xf < xfibers.end()) {
                ...
                if (add_vertex(*xf, *yf, xi, yi,
                               INT)) { // add_vertex returns false if vertex already exists
                    xi->intersections_fibers.insert(yf);
                    yi->intersections_fibers.insert(xf);
                }
...
// crossing_x fiber
bool SmartWeave::crossing_x(Fiber& yf, std::vector<Interval>::iterator& yi, Interval& xi,
                            Fiber& xf) {
...
// crossing_y fiber
bool SmartWeave::crossing_y(Fiber& xf, std::vector<Interval>::iterator& xi, Interval& yi,
                            Fiber& yf) {
...
// add_vertex
bool SmartWeave::add_vertex(Fiber& xf, Fiber& yf, std::vector<Interval>::iterator xi,
                            std::vector<Interval>::iterator yi, enum VertexType type) {
...
void SmartWeave::add_all_edges() {
    ...
    for (Vertex& vertex : vertices) {
        if ((g[vertex].type == INT) || (g[vertex].type == FULLINT)) {
            ...
            boost::tie(x_u, x_l) = find_neighbor_vertices(VertexPair(vertex, g[vertex].position.x),
                                                          *(g[vertex].xi), false);
            boost::tie(y_u, y_l) = find_neighbor_vertices(VertexPair(vertex, g[vertex].position.y),
                                                          *(g[vertex].yi), false);
            ...
            for (adj_itr = adjacent_vertices.begin(); adj_itr < adjacent_vertices.end();
                 adj_itr++) {
                Edge in, out;
        ...
        /*else if( g[vertex].type == FULLINT ) {
            std::vector<Vertex> adjacent_vertices;
            Vertex x_u, x_l, y_u, y_l;
            boost::tie( x_u, x_l ) = find_neighbor_vertices( VertexPair(vertex,
        g[vertex].position.x), *(g[vertex].xi), false ); boost::tie( y_u, y_l ) =
        find_neighbor_vertices( VertexPair(vertex, g[vertex].position.y), *(g[vertex].yi), false );
        ...
        { if( !hedi::has_edge( *adj_itr, vertex, g ) ) { in_edges.push_back( hedi::add_edge(
        *adj_itr, vertex, g ) ); out_edges.push_back( hedi::add_edge( vertex, *adj_itr, g ) );
                    }
                    else {
