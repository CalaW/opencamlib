...
public:
    ...
    /// \brief drop the MillingCutter at Point cl down along the z-axis until it makes contact with
    /// Triangle t. This function calls vertexDrop, facetDrop, and edgeDrop to do its job. Follows
    /// the template-method, or "self-delegation" design pattern. if cl.z is too low, updates cl.z
    /// so that the cutter does not cut Triangle t.
...
protected:
    ...
    bool generalFacetPush(double normal_length, double center_height, double xy_normal_length,
                          const Fiber& fib, Interval& i, const Triangle& t) const;
    ...
    /// This is called when the contact is above the sphere/toroid/cone shaped lower part of the
    /// cutter
    ...
    virtual bool generalEdgePush(const Fiber& f, Interval& i, const Point& p1,
                                 const Point& p2) const {
        return false;
    }

    /// CCPoint calculation and interval update for push-cutter
    bool calcCCandUpdateInterval(double t, double ccv, const Point& q, const Point& p1,
                                 const Point& p2, const Fiber& f, Interval& i, double height,
                                 CCType cctyp) const;
