...
BullCutter::BullCutter(double d, double r, double l) {
    ...
    radius2 = r;
    assert(radius1 > 0.0); // corner radius
    length = l;
    assert(l > 0.0);
...
CC_CLZ_Pair BullCutter::singleEdgeDropCanonical(const Point& u1, const Point& u2) const {
    if (isZero_tol(u1.z - u2.z)) { // horizontal edge special case
        ...
        int iters = e.solver_brent(); // numerical solver that searches for a point on the ellipse
                                      // such that a radius1-length normal-vector from this
                                      // ellipse-point is at the CL point (0,0)
        if (iters > 200)
...
//  edge handled here
bool BullCutter::generalEdgePush(const Fiber& f, Interval& i, const Point& p1,
    ...
    double tplane =
        (f.p1.z + radius2 - p1.z) / (p2.z - p1.z); // intersect edge with plane at z = ufp1.z
