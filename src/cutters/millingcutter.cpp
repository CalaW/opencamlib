...
bool MillingCutter::facetDrop(CLPoint& cl, const Triangle& t)
    const {                      // Drop cutter at (cl.x, cl.y) against facet of Triangle t
    ...
    if ((isZero_tol(normal.x)) && (isZero_tol(normal.y))) { // horizontal plane special case
        ...
        cc_tmp.z = (1.0 / normal.z) *
                   (-d - normal.x * cc_tmp.x - normal.y * cc_tmp.y); // cc-point lies in the plane.
        cc_tmp.type = FACET;
...
// we can test a virtual CylCutter with radius VR against an infinite ER-radius cylinder around the
// edge. This reduces to a 2D problem in the XY plane, where section of the ER-radius cylinder is an
// ellipse. in the general case CL lies on an offset ellipse with offset OR. The general cases only
// applies for BullCutter(R1,R2) where R1 is the shaft radius and R2 is the corner radius. For
// CylCutter and BallCutter there are simple analytic solutions and an offset ellipse approach is
// not required.
//
// The dual problem for each cutter type is:
//
//      CylCutter(R): VR = R, ER = 0, OR=R  (the z-position of VR is the same as for CylCutter)
//     BallCutter(R): VR = 0, ER = R, OR=0 (the z-position of VR is a distance R up from the tip of
//     BallCutter)
// BullCutter(R1,R2): VR=R1-R2, ER=R2, OR=R1-R2 (z-position of VR is a distance R2 up from the tip
// of BullCutter)
//
...
bool MillingCutter::singleEdgeDrop(CLPoint& cl, const Point& p1, const Point& p2, double d) const {
    ...
    Point u1((p1 - sc).dot(vxy), d,
             p1.z); // d, the distance to line, is the y-coord in the rotated system
...
bool MillingCutter::singleVertexPush(const Fiber& f, Interval& i, const Point& p,
                                     CCType cctyp) const {
    bool result = false;
...
bool MillingCutter::facetPush(const Fiber& fib, Interval& i, const Triangle& t) const {
    return generalFacetPush(this->normal_length, this->center_height, this->xy_normal_length, fib,
                            i, t);
}

// general purpose facetPush
bool MillingCutter::generalFacetPush(double normal_length, double center_height,
                                     double xy_normal_length, const Fiber& fib, Interval& i,
    ...
    //   find a point on the plane from which radius2*normal+radius1*xy_normal lands on the
    //   fiber+radius2*Point(0,0,1) (u,v) locates a point on the triangle facet    v0+
    //   u*(v1-v0)+v*(v2-v0)    u,v in [0,1] t locates a point along the fiber:             p1 +
    //   t*(p2-p1)             t in [0,1]
    //
    ...
    //
    //   v0x + u*(v1x-v0x) + v*(v2x-v0x) + r2*nx + r1*xy_n.x  = p1x + t*(p2x-p1x) p2x-p1x==0 for
    //   Y-fiber v0y + u*(v1y-v0y) + v*(v2y-v0y) + r2*ny + r1*xy_n.y  = p1y + t*(p2y-p1y) p2y-p1y==0
    //   for X-fiber v0z + u*(v1z-v0z) + v*(v2z-v0z) + r2*nz              = p1z + t*(p2z-p1z) + r2
    //   (p2z-p1z)==0 for XY-fibers!! X-fiber: v0x + u*(v1x-v0x) + v*(v2x-v0x) + r2*nx + r1*xy_n.x
    //   = p1x + t*(p2x-p1x) v0y + u*(v1y-v0y) + v*(v2y-v0y) + r2*ny + r1*xy_n.y  = p1y solve these
    //   two for (u,v) v0z + u*(v1z-v0z) + v*(v2z-v0z) + r2*nz              = p1z + r2 and
    //   substitute above for t or [ (v1y-v0y)    (v2y-v0y) ] [ u ] = [ -v0y - r2*ny - r1*xy_n.y +
    //   p1y     ] [ (v1z-v0z)    (v2z-v0z) ] [ v ] = [ -v0z - r2*nz + p1z + r2            ]
    //
    //   Y-fiber:
    ...
    if (fib.p1.y == fib.p2.y) { // XFIBER
        ...
        double tval = (1.0 / (fib.p2.x - fib.p1.x)) *
                      (t.p[0].x + normal_length * normal.x + xy_normal_length * xy_normal.x -
                       fib.p1.x + u * (t.p[1].x - t.p[0].x) + v * (t.p[2].x - t.p[0].x));
        ...
        double tval = (1.0 / (fib.p2.y - fib.p1.y)) *
                      (t.p[0].y + normal_length * normal.y + xy_normal_length * xy_normal.y -
                       fib.p1.y + u * (t.p[1].y - t.p[0].y) + v * (t.p[2].y - t.p[0].y));
        if (tval < 0.0 || tval > 1.0) {
            std::cout << "MillingCutter::facetPush() tval= " << tval << " error!?\n";
            std::cout << " (most probably a user error, the fiber is too short compared to the STL "
                         "model?)\n";
        }
        assert(tval > 0.0 && tval < 1.0);
        i.update(tval, cc);
...
bool MillingCutter::singleEdgePush(const Fiber& f, Interval& i, const Point& p1,
...
bool MillingCutter::shaftEdgePush(const Fiber& f, Interval& i, const Point& p1,
                                  const Point& p2) const {
    // push cutter along Fiber f in contact with edge p1-p2
    // contact with cylindrical cutter shaft
    double u, v;
    bool result = false;
    if (xy_line_line_intersection(p1, p2, u, f.p1, f.p2,
                                  v)) { // find XY-intersection btw fiber and edge
        ...
        if (xy_line_line_intersection(q1, q2, u_cc, f.p1, f.p2, t_cl)) {
            ...
            if (calcCCandUpdateInterval(t_cl1, u_cc, q, p1, p2, f, i, f.p1.z + center_height,
                                        EDGE_SHAFT))
                result = true;
            if (calcCCandUpdateInterval(t_cl2, -u_cc, q, p1, p2, f, i, f.p1.z + center_height,
                                        EDGE_SHAFT))
                result = true;
        }
    }
...
bool MillingCutter::horizEdgePush(const Fiber& f, Interval& i, const Point& p1,
                                  const Point& p2) const {
    bool result = false;
...
bool MillingCutter::calcCCandUpdateInterval(double t, double u, const Point& q, const Point& p1,
                                            const Point& p2, const Fiber& f, Interval& i,
                                            double height, CCType cctyp) const {
