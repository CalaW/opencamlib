...
// because this checks for contact with both the tip and the circular edge it is hard to move to the
// base-class we either hit the tip, when the slope of the plane is smaller than angle or when the
// slope is steep, the circular edge between the cone and the cylindrical shaft
...
CC_CLZ_Pair ConeCutter::singleEdgeDropCanonical(const Point& u1, const Point& u2) const {
    ...
    // 1) if abs(m) <= abs(mu)  we contact the curve at xp = sign(m) * sqrt( R^2 m^2 d^2 / (h^2 -
    // R^2 m^2) ) 2) if abs(m) > abs(mu) there is contact with the circular edge at +/- xu
    double ccu;
    if (hyperbola_case) {
...
bool ConeCutter::generalEdgePush(const Fiber& f, Interval& i, const Point& p1,
    ...
    // idea: as the ITO-cone slides along the edge it will pierce a z-plane at the height of the
    // fiber the shaped of the pierced area is either a circle if the edge is steep or a
    // 'half-circle' + cone shape if the edge is shallow (ice-cream cone...) we can now intersect
    // this 2D shape with the fiber and get the CL-points.
    ...
    if (d <= radius) {
        ...
        double det =
            (f.p1.x - p_base.x) * (f.p2.y - p_base.y) - (f.p2.x - p_base.x) * (f.p1.y - p_base.y);
        ...
        if (discr >= 0.0) {
            if (discr == 0.0) { // tangent case
                ...
                Point p_tang(x_tang + p_base.x,
                             y_tang + p_base.y); // translate back from (0,0) system!
    ...
    if (L > radius) {
        ...
        bool case3 =
            (dist < fabs(radius - r_tang)); // no solution. one circle is contained in the other
        ...
        if (case1 || case2 || case3 || case4) {
            ...
            Point v2 =
                p_base +
                (a / dist) * pd; // v2 is the point where the line through the circle intersection
...
bool ConeCutter::cone_CC(const Point& tang, const Point& tip, const Point& base, const Point& p1,
                         const Point& p2, const Fiber& f, Interval& i) const {
    double u, t;
...
bool ConeCutter::circle_CC(double t, const Point& p1, const Point& p2, const Fiber& f,
                           Interval& i) const {
    // cone base circle is center_height above fiber
    double t_cc = (f.p1.z + center_height - p1.z) /
                  (p2.z - p1.z); // t-parameter of the cc-point, center_height above fiber
