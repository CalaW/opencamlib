...
AlignedEllipse::AlignedEllipse(Point& centerin, double ain, double bin, double offsetin,
...
Point Ellipse::ePoint1() const { return this->ePoint(EllipsePosition1); }

Point Ellipse::ePoint2() const { return this->ePoint(EllipsePosition2); }

Point Ellipse::oePoint1() const { return this->oePoint(EllipsePosition1); }

Point Ellipse::oePoint2() const { return this->oePoint(EllipsePosition2); }
...
Point AlignedEllipse::ePoint(const EllipsePosition& pos) const {
    Point p = center + a * pos.s * major_dir +
              b * pos.t * minor_dir; // point of ellipse is:  center + a s + b t
    return p;
}
...
bool Ellipse::find_EllipsePosition2() { // a horrible horrible function... :(
    ...
    if (fabs(this->error(this->EllipsePosition2)) < err1 + OE_ERROR_TOLERANCE) {
        if ((fabs(this->EllipsePosition2.s - this->EllipsePosition1.s) > 1E-8) ||
            (fabs(this->EllipsePosition2.t - this->EllipsePosition1.t) > 1E-8)) {
#ifdef DEBUG_SOLVER
            std::cout << "2nd: (s, t)= " << this->EllipsePosition2
                      << " oePoint()= " << this->oePoint(this->EllipsePosition2)
                      << " e=" << this->error(this->EllipsePosition2) << "\n";
#endif
            return true;
        }
    }
...
int Ellipse::solver_brent() {
    int iters = 1;
    EllipsePosition apos,
        bpos; // Brent's method requires bracketing the root in [apos.diangle, bpos.diangle]
    apos.setDiangle(0.0);
    assert(apos.isValid());
    bpos.setDiangle(3.0);
    assert(bpos.isValid());
    ...
    double dia_sln =
        brent_zero(apos.diangle, bpos.diangle, 3E-16, OE_ERROR_TOLERANCE, this); // brent_zero.hpp
    EllipsePosition1.setDiangle(dia_sln);
    assert(EllipsePosition1.isValid());
    // because we only work with the y-coordinate of the offset-ellipse-point, there are two
    // symmetric solutions
    find_EllipsePosition2();
    return iters;
}
...
bool AlignedEllipse::aligned_solver(const Fiber& f) {
    ...
    // find position(s) where ellipse tangent is parallel to fiber. Here error() will be
    // minimized/maximized. tangent at point is:  -a t + b s = -a*major_dir*t + b*minor_dir*s -at
    // ma.y + bs mi.y = 0   for X-fiber s = sqrt(1-t^2)
    ...
    if (found_positive) {
        if (found_negative) {
            assert(this->error(apos.diangle) * this->error(bpos.diangle) <
                   0.0); // root is now bracketed.
            ...
            // FIXME. The allowed error should probably be in proportion to the difficulty of the
            case.

            if (!isZero_tol( error(EllipsePosition1.diangle) )) {
                std::cout << "AlignedEllipse::aligned_solver() ERROR \n";
                std::cout << "error(EllipsePosition1.diangle)= "<< error(EllipsePosition1.diangle)
            << " (expected zero)\n";
...
Point Ellipse::ePointHi() const { return ePoint(EllipsePosition_hi); }
...
void Ellipse::print_solutions() {
    std::cout << "1st: (s, t)= " << EllipsePosition1 << " oePoint()= " << oePoint(EllipsePosition1)
              << " e=" << error(EllipsePosition1) << "\n";
    std::cout << "2nd: (s, t)= " << EllipsePosition2 << " oePoint()= " << oePoint(EllipsePosition2)
              << " e=" << error(EllipsePosition2) << "\n";
}
