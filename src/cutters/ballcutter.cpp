...
bool BallCutter::generalEdgePush(const Fiber& f, Interval& i, const Point& p1,
    ...
    const Point ufp2 =
        f.p2 + Point(0, 0, radius); // and intersect it with a cylinder around the edge p1-p2
...
bool BallCutter::calcCCandUpdateInterval(double t, const Point& p1, const Point& p2, const Fiber& f,
                                         Interval& i) const {
    Point cl_center = f.point(t) + Point(0, 0, radius); // center of ball is here
    CCPoint cc_tmp =
        cl_center.closestPoint(p1, p2); // cc-point on the edge, point on edge closest to center
