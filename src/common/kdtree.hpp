...
template <class BBObj> class KDTree {
public:
    KDTree() { root = nullptr; }
    virtual ~KDTree() {
...
protected:
    ...
    KDNode<BBObj>* build_node(const std::list<BBObj>* tris, // triangles
        ...
        if ((tris->size() <= bucketSize) ||
            isZero_tol(spr->val)) { // then return a bucket/leaf node
    ...
    Spread* calc_spread(const std::list<BBObj>* tris) {
        ...
        if (tris->empty()) {
            ...
            // while(!spreads.empty()) delete spreads.back(), spreads.pop_back(); // delete the
            // others
            // std::cout << "calc_spread() done\n";
            return new Spread(dimensions[maxM], maxval[dimensions[maxM]] - minval[dimensions[maxM]],
