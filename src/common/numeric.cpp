...
#ifdef _WIN32 // Windows platform problem: error C3861: 'isnan': identifier not found, use the Boost
              // version instead
...
#endif
        std::cout << "numeric::xyVectorToDiangle() error (x,y)= (" << x << " , " << y
                  << " ) and diangle=" << diangle << "\n";
        assert(0);
    }
    return diangle;
...
bool two_by_two_solver(const double& a, const double& b, const double& c, const double& d,
                       const double& e, const double& f, double& u, double& v) {
    //  [ a  b ] [u] = [ e ]
    //  [ c  d ] [v] = [ f ]
    // matrix inverse is
    //          [ d  -b ]
...
bool xy_line_line_intersection(const Point& p1, const Point& p2, double& v, const Point& p3,
                               const Point& p4, double& t) {
    // p1 + v*(p2-p1) = p3 + t*(p4-p3)
    // =>
    // [ (p2-p1).x  -(p4-p3).x ] [ v ]  = [ (p3-p1).x ]
    // [ (p2-p1).y  -(p4-p3).y ] [ t ]  = [ (p3-p1).y ]
    return two_by_two_solver(p2.x - p1.x, -(p4.x - p3.x), p2.y - p1.y, -(p4.y - p3.y),
                             (p3.x - p1.x), p3.y - p1.y, v, t);
}
